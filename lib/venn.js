(function(venn) {
    ...
    venn.venn = function(sets, overlaps, parameters) {
        ...
        var sets1 = sets;
        sets1 = initialLayout(sets1, overlaps);
        var initial = new Array(2*sets1.length);
        for (var i = 0; i < sets1.length; ++i) {
            initial[2 * i] = sets1[i].x;
            initial[2 * i + 1] = sets1[i].y;
        }
        var totalFunctionCalls = 0;
        var solution = venn.fmin(
            function(values) {
                totalFunctionCalls += 1;
                var current = new Array(sets1.length);
                for (var i = 0; i < sets1.length; ++i) {
                    current[i] = {x: values[2 * i],
                                  y: values[2 * i + 1],
                                  radius : sets1[i].radius,
                                  size : sets1[i].weight
                              };
                }
        ...
        for (i = 0; i < sets1.length; ++i) {
            sets1[i].x = positions[2 * i];
            sets1[i].y = positions[2 * i + 1];
        }
        return sets1;
    };
    ...
    venn.getDistanceMatrix = function(sets, overlaps) {
        ...
        for (i = 0; i < overlaps.length; ++i) {
            ...
            var left = current.sets[0],
                ...
                r1 = Math.sqrt(sets[left].weight / Math.PI),
                r2 = Math.sqrt(sets[right].weight / Math.PI),
                distance = venn.distanceFromIntersectArea(r1, r2, current.weight);
    ...
    venn.greedyLayout = function(sets, overlaps) {
        ...
        for (var i = 0; i < sets.length; ++i) {
            setOverlaps[i] = [];
            sets[i].radius = Math.sqrt(sets[i].weight / Math.PI);
            sets[i].x = sets[i].y = 0;
        }
        for (i = 0; i < overlaps.length; ++i) {
            var current = overlaps[i];
            ...
            setOverlaps[left].push ({set:right, size:current.weight, weight:weight});
            setOverlaps[right].push({set:left,  size:current.weight, weight:weight});
        }
        var mostOverlapped = [];
        for (var set in setOverlaps) {
            if (setOverlaps.hasOwnProperty(set)) {
                ...
                for (i = 0; i < setOverlaps[set].length; ++i) {
                    size += setOverlaps[set][i].weight * setOverlaps[set][i].weight;
                }
    ...
    venn.lossFunction = function(sets, overlaps) {
        ...
        for (var i = 0; i < overlaps.length; ++i) {
            ...
            output += weight * (overlap - area.weight) * (overlap - area.weight);
        }
        return output;
    };
    ...
    venn.wrapText = function() {
        var text = d3.select(this),
            data = text.datum(),
            width = data.radius;
            
            var words = data.name.split(/\s+/).reverse(),
            maxLines = 3,
            minChars = (data.name.length + words.length) / maxLines,
            word = words.pop(),
            line = [word],
            joined,
            lineNumber = 0,
    ...
    venn.computeTextCenters = function (sets, width, height, diagram) {
        var sums = [];
        for (var i = 0; i < sets.length; ++i) {
