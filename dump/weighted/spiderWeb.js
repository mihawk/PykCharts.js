PykCharts.weighted.spiderWeb = function (options) {
    ...
    this.execute = function () {
        ...
        that.axisTitle = optional && optional.axisTitle ? optional.axisTitle : twoDimensionalCharts.spiderweb.axisTitle;
        that.bubbleRadius = options.spiderweb && _.isNumber(options.spiderweb.radius) ? options.spiderweb.radius : twoDimensionalCharts.spiderweb.radius;
        that.outerRadius = options.spiderweb && _.isNumber(options.spiderweb.outer_radius) ? options.spiderweb.outer_radius : twoDimensionalCharts.spiderweb.radius; 
        that.innerRadius = 0;
            ...
            axisTitle : function () {
                if(PykCharts.boolean(that.axisTitle)) {
                    ...
                    spider_text
                        ...
                        // .attr("x", function(d, i){return that.width/2*(1-0.2*Math.sin(i*2*Math.PI/that.length))+350*Math.sin(i*2*Math.PI/that.length);})
                        // .attr("y", function(d, i){return that.height/2*(1-0.65*Math.cos(i*2*Math.PI/that.length))-80*Math.cos(i*2*Math.PI/that.length);})
