PykCharts.twoD.area = function (options){
	var that = this;
	...
	this.render = function (){
		...
		if(that.mode === "default") {
		    ...
		    that.k.crossHair(that.svg)
				    .xAxis(that.svg,that.gxaxis,that.xScale)
						.yAxis(that.svg,that.gyaxis,that.yScale);
  		}
	};
	...
	this.optional_feature = function (){
        ...
        if(that.xAxisDataFormat === "number") {
          ...
          that.lineMargin = 0;
				...
				// 		.domain([0, d3.max(that.stacked_new_data, function(d) { return d3.max(d.data, function(k) { return k.y0 + k.y; }); })])
        		...
        		that.svg.select("#"+type)
					...
					if(that.type === "areaChart") {
						that.svg
							...
							.on("mousemove", function(){
								that.mouseEvent.crossHairPosition(that.data,that.xScale,that.svg.select("#"+type));
					  	});
					}
				}
				else {
					...
					if(that.type === "areaChart") {
						that.svg
					  	...
					  	.on("mousemove", function(){
					  		that.mouseEvent.crossHairPosition(that.data,that.xScale,that.dataLineGroup[0]);
					  	});
					}
				}
				return this;
