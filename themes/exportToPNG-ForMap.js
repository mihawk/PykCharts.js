...
casper.start('http://localhost/PykCharts/themes/exportToPNG.html?slug='+chart_name+'&data='+data+'&map_code='+chart_data, function() {
    this.waitForSelector("#chart_container0", (function() {
        for (var i = 0; i < 11; i++) {
            this.captureSelector(chart_data+"-"+chart_name+"-"+theme_list[i]+'.png', '#chart_container'+i);
