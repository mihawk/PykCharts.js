        ...
        var optional = {
            ...
            clubData : function () {
                if(PykCharts.boolean(that.clubData_enable)){
                    ...
                    that.data.sort(function (a,b) { return b.weight - a.weight; });
                    while(that.new_data.length<that.clubData_maximumNodes-1){
                        for(i=0;i<clubdata_content.length;i++){
                            if(that.data[k].name.toUpperCase() === clubdata_content[i].toUpperCase()){
                                k++;
                            }
                    ...
                    var sortfunc = function (a,b) { return b.weight - a.weight; };
                    while(that.new_data.length > that.clubData_maximumNodes){
                        that.new_data.sort(sortfunc);
                        var a=that.new_data.pop();
                    }
                    var other_span = { "name":that.clubData_text, "weight": weight, "color": that.clubData_color, "tooltip": that.clubData_tooltip };

                    if(that.new_data.length < that.clubData_maximumNodes){
