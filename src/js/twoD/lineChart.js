PykCharts.twoD.line = function (options){
	...
	this.optional_feature = function (){
		var optional = {
			...
			createSvg: function (i){
				...
				that.svg = d3.select("#tooltip-svg"+i).append("svg:svg")
					.attr("id","svg" + i)
					.attr("width",that.w)
			...
			createChart : function (evt) {
				...
				// that.pt_circle.attr("id","pt-line"+that.svg.attr("id"));
				// that.start_pt_circle = $("#"+that.pt_circle.attr("id")).clone().appendTo("#"+that.svg.attr("id"));
				// that.start_pt_circle
				// 		.attr("id","start-pt-line"+that.svg.attr("id"));
				// that.end_pt_circle = $("#"+that.start_pt_circle.attr("id")).clone().appendTo("#"+that.svg.attr("id"));
			 //  	that.end_pt_circle
			 //  			.attr("id","end-pt-line"+that.svg.attr("id"));
			 //  	that.pt_circle.remove();
			    ...
			    if(evt === "liveData"){
				   ...
				   //    		.duration(that.transitions[that.transition.enable]().duration())
							// 		.attr("transform", "translate("+ that.lineMargin +",100)")
					...
					if(!PykCharts.boolean(that.multiple_containers)) {
						for (var i = 0;i < that.new_data_length;i++) {
						  ...
						  if(that.type === "multiLineChart") {
						  	// that.dataLineGroup[i]
						   //    	.style("stroke", function(d) { return that.chartColor; });

						  	that.dataLineGroup[i].on("click",function (d) {
						  			that.selected_line = d3.event.target;
										that.selected_line_data = that.selected_line.__data__;
										that.selected_line_data_len = that.selected_line_data.length;
										// console.log(that.selected_line_data,that.selected_line_data_len);
	...
	this.updateSelectedLine = function (lineid) {
		...
		var start_x = (that.xScale(that.selected_line_data[0].x) + that.lineMargin + that.margin.left),
				...
				end_y = (that.yScale(that.selected_line_data[(that.selected_line_data_len - 1)].y) + that.margin.top),start,end;
		d3.selectAll("#start-pt-line"+svgid)
			.select("circle")
			.style("visibility","visible");
		d3.selectAll("#end-pt-line"+svgid)
			.select("circle")
			.style("visibility","visible");
		that.start_pt_circle.show();
		that.start_pt_circle
				.select("circle")
				// .attr("fill",that.highlightColor)
				.attr("class","bullets")
				// .css({fill:that.highlightColor})
